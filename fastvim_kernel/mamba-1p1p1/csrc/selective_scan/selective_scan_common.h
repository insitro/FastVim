...
template<typename Ktraits>
inline __device__ void load_to_sharedmem_with_D(
        typename Ktraits::input_t *global_obj,
        typename Ktraits::input_t *local_vals,
        int num_valid, float *D_val) {

    // ALL threads need to read at once for memory coalescing to occur.
    // Last read may not be coalesced if it's not evenly divisible.
    int num_to_read = num_valid / blockDim.x;

    // These are all coalesced reads.
    for (int i = 0; i < num_to_read; i++) {
        int idx = threadIdx.x * num_to_read + i;
        if (D_val == nullptr) {
            local_vals[idx] = 0.;
        } else {
            local_vals[idx] = *D_val * global_obj[idx];
        }
    }

    // Remaining items are fewer than threads so have each thread just read its one.
    int leftover_idx = threadIdx.x + num_to_read * blockDim.x;
    if (leftover_idx < num_valid) {
        if (D_val == nullptr) {
            local_vals[leftover_idx] = 0.;
        } else {
            local_vals[leftover_idx] =  *D_val * global_obj[leftover_idx];
        }
    }
}

template<typename Ktraits>
inline __device__ void save_from_sharedmem(
        typename Ktraits::input_t *shared_obj,
        typename Ktraits::input_t *global_obj, int num_valid) {

    // ALL threads need to read at once for memory coalescing to occur.
    // Last read may not be coalesced if it's not evenly divisible.
    int num_to_write = num_valid / blockDim.x;

    // These are all coalesced writes.
    for (int i = 0; i < num_to_write; i++) {
        int idx = threadIdx.x * num_to_write + i;
        global_obj[idx] = shared_obj[idx];
    }

    int leftover_idx = threadIdx.x + num_to_write * blockDim.x;
    if (leftover_idx < num_valid) {
        global_obj[leftover_idx] =  shared_obj[leftover_idx];
    }
}
...
template<typename Ktraits>
inline __device__ void store_decompressed_output(
        // Out needs to point to the right spot for this thread
        typename Ktraits::input_t *out,
        float *vals, // this thread's data
        unsigned int compression_factor,
        int num_to_write) {
    // Probably not as efficient as using  BlockstoreT::TempStore but that requires
    // data already be partitioned across threads.
    // Testing with compression_factor = 1 will reveal how much of a slowdown this is.

    int in_idx = 0;
    int out_idx = 0;

    // Each thread writes kNitems at most.
    for (int i = 0 ; i < num_to_write; i++) {
        for (int c = 0; c < compression_factor; c++) {
            out[out_idx + c] = vals[i];
        }
        out_idx += compression_factor;
    }
}
