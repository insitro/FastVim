...
void set_ssm_params_fwd(SSMParamsBase &params,
                        ...
                        const size_t dim,
                        const size_t seqlen,
                        const unsigned int compression_factor,
                        const size_t dstate,
    ...
    params.u_c_ptr = u_compressed.data_ptr();
    ...
    // Input tensors must be contiguous on seqlen dimension (2), but
    // not necessarily on others. Track strides for these dimensions to
    // correctly index into the tensor for each kernels' sequence.
    params.u_batch_stride = u.stride(0);
    params.u_d_stride = u.stride(1);

    params.u_c_batch_stride = u_compressed.stride(0);
    params.u_c_d_stride = u_compressed.stride(1);
...
void set_ssm_params_bwd(SSMParamsBwd &params,
    ...
    set_ssm_params_fwd(params, batch, dim, seqlen, compression_factor,
                       dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
                       u, u_compressed,
                       delta, A, B, C,
                       has_z ? out : dout, has_z ? z : dout,
    ...
    params.du_c_ptr = du_compressed.data_ptr();
    ...
    params.du_c_batch_stride = du_compressed.stride(0);
    params.du_c_d_stride = du_compressed.stride(1);
...
// Robin: this is selective_scan.fwd()
std::vector<at::Tensor>
selective_scan_fwd(const at::Tensor &u, const at::Tensor &u_compressed,
                  const at::Tensor &delta,
    ...
    TORCH_CHECK(input_type == u_compressed.scalar_type());
    ...
    TORCH_CHECK(u_compressed.is_cuda());
    ...
    TORCH_CHECK(u_compressed.stride(-1) == 1 || u_compressed.size(-1) == 1);
    ...
    const int seqlen_compressed = u_compressed.sizes()[2];
    ...
    TORCH_CHECK(seqlen % seqlen_compressed == 0,
      "Compressed sequence must have length that is an integer multiple of uncompressed sequence.")
    const int compression_factor = seqlen / seqlen_compressed;

    CHECK_SHAPE(u_compressed, batch_size, dim, seqlen_compressed);
    CHECK_SHAPE(u, batch_size, dim, seqlen);
    CHECK_SHAPE(delta, batch_size, dim, seqlen_compressed);
    CHECK_SHAPE(A, dim, dstate);
    if (!is_variable_B) {
        CHECK_SHAPE(B, dim, dstate);
    } else {
        CHECK_SHAPE(B, batch_size, n_groups, dstate, !is_complex ? seqlen_compressed : seqlen_compressed * 2);
        TORCH_CHECK(B.stride(-1) == 1 || B.size(-1) == 1);
    }
    if (!is_variable_C) {
        CHECK_SHAPE(C, dim, dstate);
    } else {
        CHECK_SHAPE(C, batch_size, n_groups, dstate, !is_complex ? seqlen_compressed: seqlen_compressed * 2);
...
# if 0
    if (has_z) {
        ...
        out_z = torch::empty(
                {batch_size, dim, seqlen * compression_factor},
                z.options()
                );
    }
#endif

    const int n_chunks = (seqlen_compressed + 2048 - 1) / 2048;
    // const int n_chunks = (seqlen + 1024 - 1) / 1024;
    // at::Tensor out = torch::empty_like(u);
    // Right now u has BHL layout and delta has HBL layout, and we want out to have HBL layout
    //CHECK_SHAPE(batch_size, dim, seqlen);

    at::Tensor out;
    out = torch::empty(
           {batch_size, dim, seqlen},
           delta.options()
          );
    // end changes
    ...
    set_ssm_params_fwd(params, batch_size, dim, seqlen_compressed, compression_factor,
                       dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
                       u, u_compressed,
                       delta, A, B, C,
                       out, z, out_z,
    ...
    // Robin - float32 only for faster compile time.
    using input_t = float;
...
std::vector<at::Tensor>
selective_scan_bwd(const at::Tensor &u, const at::Tensor &u_compressed,
                  const at::Tensor &delta,
                  const at::Tensor &A, const at::Tensor &B, const at::Tensor &C,
                  const c10::optional<at::Tensor> &D_,
                  const c10::optional<at::Tensor> &z_,
...
# if 0
    ...
    at::Tensor du_compressed = torch::empty_like(u_compressed);
    ...
    SSMParamsBwd params;
    set_ssm_params_bwd(params, batch_size, dim, seqlen_compressed, compression_factor,
                       dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
                       u, u_compressed,
                       delta, A, B, C, z, out, out_z,
                       D_.has_value() ? D_.value().data_ptr() : nullptr,
                       delta_bias_.has_value() ? delta_bias_.value().data_ptr() : nullptr,
                       x_.has_value() ? x_.value().data_ptr() : nullptr,
                       dout, du, du_compressed,
                       ddelta, dA, dB, dC, dz,
    ...
    */
    std::vector<at::Tensor> result = {du, du_compressed, ddelta, dA, dB.to(B.dtype()), dC.to(C.dtype()), dD, ddelta_bias};
