...
class FastVim_MambaInnerFnNoOutProj_withoutZ(torch.autograd.Function):
    @staticmethod
    @custom_fwd
    def forward(
        ctx,
        ...
        if collapse_method == "mean":
            conv1d_out_compressed = conv1d_out.reshape(pre_x_shape).mean(
                dim=3
            )  # Shape: (B, d, 14)
            if scaling_factor != 1:
                conv1d_out_compressed *= scaling_factor

        L_compressed = conv1d_out_compressed.shape[-1]

        x_dbl = F.linear(
            rearrange(conv1d_out_compressed, "b d l -> (b l) d"), x_proj_weight
        )  # (bl d)
        delta = rearrange(
        ...
        if B is None:  # variable B
            ...
            if not A.is_complex():
                B = rearrange(
                    B, "(b l) dstate -> b 1 dstate l", l=L_compressed
                ).contiguous()
            else:
                B = rearrange(
                    B, "(b l) (dstate two) -> b 1 dstate (l two)", l=L_compressed, two=2
                ).contiguous()
        else:
            if B.stride(-1) != 1:
                B = B.contiguous()
        if C is None:  # variable C
            ...
            if not A.is_complex():
                C = rearrange(
                    C, "(b l) dstate -> b 1 dstate l", l=L_compressed
                ).contiguous()
            else:
                C = rearrange(
                    C, "(b l) (dstate two) -> b 1 dstate (l two)", l=L_compressed, two=2
                ).contiguous()
        else:
            if C.stride(-1) != 1:
                C = C.contiguous()
        ...
        out, scan_intermediates, *rest = selective_scan_cuda.fwd(
            conv1d_out_compressed,
            delta,
            A,
            B,
            C,
        ...
        out = out.repeat_interleave(num_of_col, dim=2)
        out += D.unsqueeze(-1) * conv1d_out

        ctx.delta_softplus = delta_softplus
        ctx.num_of_col = num_of_col
        ctx.collapse_method = collapse_method
        ctx.scaling_factor = scaling_factor
        ctx.pre_x_shape = pre_x_shape

        ctx.checkpoint_lvl = checkpoint_lvl
        if (
            checkpoint_lvl >= 1
        ):  # Will recompute conv1d_out and delta in the backward pass
            conv1d_out, conv1d_out_compressed, delta = None, None, None
    ...
    def backward(ctx, dout):
        ...
        num_of_col = ctx.num_of_col
        collapse_method = ctx.collapse_method
        scaling_factor = ctx.scaling_factor
        pre_x_shape = ctx.pre_x_shape
        ...
        if ctx.checkpoint_lvl == 1:
            ...
            delta = rearrange(
                delta_proj_weight @ x_dbl[:, :delta_rank].t(),
                "d (b l) -> b d l",
                l=conv1d_out_compressed.shape[-1],
            )
        ...
        # 1. Backward pass for D.unsqueeze(-1) * conv1d_out
        dD = (
            (dout * conv1d_out).sum(dim=-1)
        ).contiguous()  # Gradient with respect to D (sum over sequence length)
        dconv1d_out = (
            dout * D.unsqueeze(-1)
        ).contiguous()  # Element-wise multiplication for conv1d_out

        # Backward pass for repeat_interleave
        dout = (
            dout.view(dout.shape[0], dout.shape[1], -1, num_of_col)
            .sum(dim=-1)
            .contiguous()
        )
        ...
        dconv1d_out_compressed, ddelta, dA, dB, dC, _, ddelta_bias, *rest = (
            selective_scan_cuda.bwd(
                conv1d_out_compressed,
                delta,
                A,
        ...
        dconv1d_out_compressed = rearrange(dconv1d_out_compressed, "b d l -> d (b l)")
        dx_proj_weight = torch.einsum(
            "Br,Bd->rd", dx_dbl, rearrange(conv1d_out_compressed, "b d l -> (b l) d")
        )
        dconv1d_out_compressed = torch.addmm(
            dconv1d_out_compressed,
            x_proj_weight.t(),
            dx_dbl.t(),
            out=dconv1d_out_compressed,
        )
        dconv1d_out_compressed = rearrange(
            dconv1d_out_compressed, "d (b l) -> b d l", b=x.shape[0], l=L_compressed
        )

        # Backward pass through the collapse method
        if collapse_method == "mean":
            if scaling_factor != 1:
                dconv1d_out_compressed *= scaling_factor

            # dconv1d_out = dconv1d_out + (dconv1d_out_compressed.unsqueeze(3).expand(-1, -1, -1, num_of_col) / num_of_col).reshape_as(conv1d_out)

            dconv1d_out = dconv1d_out + (
                dconv1d_out_compressed.repeat_interleave(num_of_col, dim=2) / num_of_col
            )

        # Backward pass through conv1d
        dx, dconv1d_weight, dconv1d_bias = causal_conv1d_cuda.causal_conv1d_bwd(
            x, conv1d_weight, conv1d_bias, dconv1d_out, None, dx, True
        )
...
class MambaInnerFnNoOutProj_withoutZ(torch.autograd.Function):
    @staticmethod
    @custom_fwd
    def forward(
        ctx,
        ...
        out, scan_intermediates, *rest = selective_scan_cuda.fwd(
            conv1d_out, delta, A, B, C, D, None, delta_bias, delta_softplus
        )
    ...
    def backward(ctx, dout):
        ...
        dx = torch.empty_like(x)  # (batch, dim, seqlen)
        # dout_y = rearrange(dout, "b l d -> b d l") # because no arrange at end of forward, so dout shape is b d l
        dconv1d_out, ddelta, dA, dB, dC, dD, ddelta_bias, *rest = (
            selective_scan_cuda.bwd(
                conv1d_out,
                delta,
                A,
...
def mamba_inner_fn_no_out_proj_withoutZ(
    x,
    conv1d_weight,
    conv1d_bias,
    x_proj_weight,
    ...
    return MambaInnerFnNoOutProj_withoutZ.apply(
        x,
        conv1d_weight,
        conv1d_bias,
        x_proj_weight,
...
def FastVim_mamba_inner_fn_no_out_proj_withoutZ(
    x,
    conv1d_weight,
    conv1d_bias,
    x_proj_weight,
    ...
    return FastVim_MambaInnerFnNoOutProj_withoutZ.apply(
        x,
        conv1d_weight,
        conv1d_bias,
        x_proj_weight,
...
# RuntimeError: function MambaInnerFnNoOutProjBackward returned an incorrect number of gradients (expected 14, got 13)
def mamba_inner_ref(
    xz,
    conv1d_weight,
    conv1d_bias,
