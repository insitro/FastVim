...
try:
    from mamba_ssm.ops.selective_scan_interface import (
        ...
        mamba_inner_fn_no_out_proj_withoutZ,
        selective_scan_fn,
    )
except ImportError:
    (
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if (
            ...
            if self.use_norm_after_ssm is True:
                ...
                out = mamba_inner_fn_no_out_proj_withoutZ(
                    x,
                    self.conv1d.weight,
                    self.conv1d.bias,
                    self.x_proj.weight,
                ...
                out_b = mamba_inner_fn_no_out_proj_withoutZ(
                    x.flip([-1]),
            ...
            if conv_state is not None:
                ...
                # Instead F.pad will pad with zeros if seqlen < self.d_conv, and truncate otherwise.
                conv_state.copy_(
                    F.pad(x_flip, (self.d_conv - x_flip.shape[-1], 0))
                )  # Update state (B D W)
            if causal_conv1d_fn is None:
                x_flip = self.act(self.conv1d_b(x_flip)[..., :seqlen])
            else:
                assert self.activation in ["silu", "swish"]
            ...
            # We want dt to have d as the slowest moving dimension
            # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
            x_dbl_b = self.x_proj_b(rearrange(x_flip, "b d l -> (b l) d"))  # (bl d)
