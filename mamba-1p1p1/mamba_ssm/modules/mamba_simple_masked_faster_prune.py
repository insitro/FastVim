...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, ids_keep, inference_params=None):
        ...
        if (
            ...
            ids_keep = ids_keep // self.num_of_col

            if self.collapse_method == "mean":
                # x_compressed, x_compressed_b = compute_row_means_constantdivide(
                #     x, x_flip, ids_keep, self.num_of_rows, self.num_of_col
                # )

                x_compressed, x_compressed_b = compute_row_means(x, x_flip, ids_keep, self.num_of_rows)
            ...
            out = torch.gather(
                out, 2, ids_keep.unsqueeze(1).expand(-1, self.d_inner, -1)
            ).contiguous()
            ...
            out_b = torch.gather(
                out_b, 2, ids_keep.unsqueeze(1).expand(-1, self.d_inner, -1)
            ).contiguous()
            out_b += self.D_b.float().unsqueeze(-1) * x_flip

            scores = torch.clamp((out + out_b.flip([-1]) / 2), min=0).sum(dim=1)
...
def compute_row_means(
    x: torch.Tensor, x_flip: torch.Tensor, ids_keep: torch.Tensor, rows: int
) -> Tuple[torch.Tensor, torch.Tensor]:
    # Convert ids_keep to row indices
    B, D, L = x.shape

    # Transpose and reshape x and x_flip to combine batch and sequence dimensions
    x_flat = x.permute(0, 2, 1).reshape(-1, D)  # Shape: (B * l, D)
    x_flip_flat = x_flip.permute(0, 2, 1).reshape(-1, D)  # Shape: (B * l, D)
    ids_flat = ids_keep.reshape(-1)  # Shape: (B * l)

    # Create batch indices
    batch_indices = (
        torch.arange(B, device=x.device).unsqueeze(1).expand(B, L).reshape(-1)
    )  # Shape: (B * l)

    # Compute linear indices for combining batch and ids_keep indices
    linear_indices = batch_indices * rows + ids_flat  # Shape: (B * l)

    # Initialize accumulation tensors
    row_sums = torch.zeros((B * rows, D), device=x.device)  # Shape: (B * rows, D)
    row_sums_b = torch.zeros_like(row_sums)
    row_counts = torch.zeros((B * rows), device=x.device)  # Shape: (B * rows)

    # Accumulate sums using index_add_
    row_sums.index_add_(0, linear_indices, x_flat)
    row_sums_b.index_add_(0, linear_indices, x_flip_flat)
    row_counts.index_add_(
        0, linear_indices, torch.ones_like(ids_flat, dtype=row_counts.dtype)
    )

    # Reshape accumulations back to (B, rows, D) and counts to (B, rows, 1)
    row_sums = row_sums.view(B, rows, D)
    row_sums_b = row_sums_b.view(B, rows, D)
    row_counts = row_counts.view(B, rows, 1)

    # Calculate means
    means = row_sums / (row_counts + 1e-6)
    means_b = row_sums_b / (row_counts + 1e-6)

    # Transpose back to (B, D, rows) if needed
    means = means.permute(0, 2, 1).contiguous()  # Shape: (B, D, rows)
    means_b = means_b.permute(0, 2, 1).contiguous()  # Shape: (B, D, rows)

    return means, means_b


@torch.jit.script
def compute_row_means_constantdivide(
    x: torch.Tensor, x_flip: torch.Tensor, ids_keep: torch.Tensor, rows: int, cols: int
) -> Tuple[torch.Tensor, torch.Tensor]:
    # Convert ids_keep to row indices
    B, D, L = x.shape
