            ...
            if self.use_norm_after_ssm is True:
                ...
                if self.collapse_method == "mean":
                    if self.scan_order == "Spatial-First":
                        ...
                        x_compressed = (
                            x.reshape(pre_x_shape).mean(dim=4).flatten(2)
                        )  # Shape: (B, d, 14)
                        x_compressed_b = (
                            x_flip.reshape(pre_x_shape).mean(dim=4).flatten(2)
                        )  # Shape: (B, d, 14)
                        ...
                        x_compressed = (
                            x.reshape(pre_x_shape).mean(dim=3).flatten(2)
                        )  # Shape: (B, d, 14)
                        x_compressed_b = (
                            x_flip.reshape(pre_x_shape).mean(dim=3).flatten(2)
                        )  # Shape: (B, d, 14)
                    ...
                    if self.scan_order == "Spatial-First":
                        ...
                        x_compressed = (
                            x.reshape(pre_x_shape).max(dim=4).values.flatten(2)
                        )  # Shape: (B, d, 14)
                        x_compressed_b = (
                            x_flip.reshape(pre_x_shape).max(dim=4).values.flatten(2)
                        )  # Shape: (B, d, 14)
                        ...
                        x_compressed = (
                            x.reshape(pre_x_shape).max(dim=3).values.flatten(2)
                        )  # Shape: (B, d, 14)
                        x_compressed_b = (
                            x_flip.reshape(pre_x_shape).max(dim=3).values.flatten(2)
                        )  # Shape: (B, d, 14)
                ...
                if self.use_our_selective_scan is False:
                    ...
                    if self.scan_order == "Spatial-First":
                        out = (
                            out.reshape(
                                (-1, self.d_inner, tokens_per_patch, self.num_of_rows)
                            )
                            .repeat_interleave(self.num_of_col, dim=3)
                            .flatten(2)
                        )
                    else:
                        out = (
                            out.reshape(
                                (-1, self.d_inner, self.num_of_rows, tokens_per_patch)
                            )
                            .repeat_interleave(self.num_of_col, dim=2)
                            .flatten(2)
                        )
