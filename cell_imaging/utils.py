...
def get_params_groups_wd(model: nn.Module) -> List[Dict[str, Any]]:
...
def get_lr_scheduler(
    ...
    """Create an array containing the DINO LR schedule.
    ...
    try:  # when pytorch lightning datamodule
        ...
        print("len(train_dataloader), world_size", len(train_dataloader), world_size)

        lr_schedule = cosine_scheduler(
            base_value,
            min_lr,
            scheduling_epochs,
            len(dataloader.train_dataloader()) // world_size,
...
def get_wd_scheduler(
    ...
    """Build the AdamW weight decay schedule.

    Parameters
    ----------
        The training configuration. Required for batch_size and world_size.
    dataloader: pl.LightningDataModule
        The dataloader being used for training.
    weight_decay: float
        The initial weight decay.
    weight_decay_end: float
        The final weight decay.
    scheduling_epochs: int
        The number of epochs to schedule for. Must be >= the number of training epochs.
    """
