...
class CellAugmentation(object):
    def __init__(
        ...
        if len(normalization_mean) == 0:
            ...
            normalize = A.Compose(
                [A.Normalize(mean_div_255, std_div_255), ToTensorV2()]
            )
        ...
        if global_resize == -1:
            random_pad_crop = A.NoOp()
        else:
            random_pad_crop = RandomPadCrop(global_resize)
...
class CellAugmentation_withmask(object):
    def __init__(
        self,
        is_train: bool,
        global_resize: int = 224,
        ...
        self.is_train = is_train
        self.normalize = A.Normalize(mean_div_255, std_div_255)
    ...
    def __call__(self, image, cell_mask) -> Union[list[torch.Tensor], torch.Tensor]:
        """
        Take a PIL image, generate its data augmented version
        """
        img = np.asarray(image)
        if self.is_train:
            out = self.global_transform1(image=img, mask=cell_mask)
            cell_mask = out["mask"]
            img = self.global_transform2(image=out["image"])["image"]

        img = self.normalize(image=img)["image"]
        img = self.to_tensor(image=img)["image"]
        cell_mask = self.to_tensor(image=cell_mask)["image"]

        if len(self.channel_mask) == 0:
            # do not mask channels
            return img, cell_mask
        else:
