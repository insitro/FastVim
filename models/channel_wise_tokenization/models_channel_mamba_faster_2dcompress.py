...
from mamba_ssm.modules.mamba_simple_channel_faster_2dcompress import Mamba
...
class PatchEmbedPerChannel(nn.Module):
    ...
    stride : int
        The stride used to extract the patches. Default is 16.
    in_chans : int
        The number of input channels. Default is 8
    embed_dim : int
        The dimension of the image embedding. Default is 768.
    hcs: bool
        If True (default), use hierarchical channel sampling during training.
    scan_order : str
        The order of scanning the image patches. Default is 'Channel-First'.
        Can be either 'Channel-First' or 'Spatial-First'.
    sort_channels : bool
        If True (default), sort the channels during hierarchical channel sampling.
    flatten : bool
        If True (default), flatten the image patches. Otherwise, keep the 2D shape.
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if (
            self.rotate_every_block is True and (self.layer_idx + 2) % 3 == 0
        ):  # rowwise scan -> colwise scan -> channel wise scan -> repeat
        ...
        if (
            self.rotate_every_block and (self.layer_idx + 2) % 3 == 0
        ):  # rowwise scan -> colwise scan -> channel wise scan -> repeat
            if self.scan_order == "Spatial-First":
                hidden_states = hidden_states.reshape(
...
def create_block(
    ...
    if (
        rotate_every_block is True and (layer_idx + 2) % 3 == 0
    ):  # rowwise scan -> colwise scan -> channel wise scan -> repeat
        mixer_cls = partial(
            Mamba,
            layer_idx=layer_idx,
