...
class MaskedAutoencoderViM(nn.Module):
    ...
    def random_masking(self, x, mask_ratio):
        ...
        # for Mamba since sequential - ******important******
    ...
    def forward_encoder(self, x, mask_ratio, inference_params=None):
        ...
        token_position = M // 2
        x = torch.cat(
            (x[:, :token_position, :], cls_tokens, x[:, token_position:, :]), dim=1
        )
    ...
    def forward_decoder(self, x, ids_restore, inference_params=None):
        ...
        x_ = torch.cat(
            [x[:, :token_position, :], x[:, token_position + 1 :, :], mask_tokens],
            dim=1,
        )  # no cls token
        x_ = torch.gather(
        ...
        cls_tokens = (
            x[:, token_position : token_position + 1, :]
            + self.decoder_pos_embed[:, :1, :]
        )

        token_position_new = x_.shape[1]
        x = torch.cat(
            [x_[:, :token_position_new, :], cls_tokens, x_[:, token_position_new:, :]],
            dim=1,
        )  # no cls token
        ...
        # remove cls token
        x = torch.cat(
            [x[:, :token_position_new, :], x[:, token_position_new + 1 :, :]], dim=1
        )  # no cls token
...
@register_model
def mae_vim_base_dec512d2b(patch_size=16, stride=16, **kwargs):
...
@register_model
def mae_vim_large_dec512d2b(patch_size=16, stride=16, **kwargs):
    model = MaskedAutoencoderViM(
        patch_size=patch_size,
        stride=stride,
        embed_dim=1024,
...
@register_model
def mae_vim_huge_dec512d2b(patch_size=16, stride=16, **kwargs):
    model = MaskedAutoencoderViM(
        patch_size=patch_size,
        stride=stride,
        embed_dim=1280,
