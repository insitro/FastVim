...
from mamba_ssm.modules.mamba_simple_masked_faster_v2 import Mamba_masked
...
class Block_masked(nn.Module):
    def __init__(
        ...
        self.rotate_indices = self.compute_rotate_indices(token_size[0], token_size[1])

    # Calculate rotate indices
    def compute_rotate_indices(self, H, W):
        rotate_indices = torch.zeros(H * W, dtype=torch.long)
        for i in range(H):
            for j in range(W):
                original_idx = i * W + j
                rotated_idx = j * H + i
                rotate_indices[original_idx] = rotated_idx
        return rotate_indices
    ...
    def forward(
        ...
        if self.rotate_every_block is True and self.layer_idx % 2 != 0:
            ...
            ids_keep = rotate_indices[ids_keep]
            rotated_ids = torch.argsort(ids_keep)

            ids_keep = torch.gather(ids_keep, 1, rotated_ids)
            rotated_ids = rotated_ids.unsqueeze(-1)
            hidden_states = torch.gather(
                hidden_states, 1, rotated_ids.repeat(1, 1, hidden_states.shape[-1])
            )
        ...
        if self.rotate_every_block is True and self.layer_idx % 2 != 0:
            hidden_states = torch.gather(
                hidden_states,
                1,
                torch.argsort(rotated_ids, -2).repeat(1, 1, hidden_states.shape[-1]),
            )
